...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf(stderr, "Usage: %s <input_image> <grayscale_out.png> <edges_out.png> [serial_baseline_seconds]\n", argv[0]);
    ...
    /* Optional 4th arg: serial baseline's total time, for Speedup/Efficiency
       (Unit 1 performance metrics), same convention as openmp_filter.c. */
    int has_baseline = (argc >= 5);
    double serial_baseline_time = has_baseline ? atof(argv[4]) : 0.0;
    ...
    /* --- Performance metrics (Unit 1 + Unit 3: GPU architecture) ---
       Speedup S = T_serial / T_gpu is straightforward and directly
       comparable to the OpenMP calculation.

       Efficiency E = S / P is trickier for a GPU: P in the classical
       formula means "number of processing units used", which for a CPU
       is simply the thread/core count. A GPU doesn't have a small fixed
       "core count" in the same sense -- it has thousands of lightweight
       CUDA cores grouped into Streaming Multiprocessors (SMs), which are
       the actual independent scheduling units on the chip (Unit 3: GPU
       architecture). Using the SM count as P gives a more architecturally
       meaningful efficiency than using the (huge, and mostly irrelevant)
       total thread count would. */
    cudaDeviceProp prop;
    ...
    if (has_baseline && serial_baseline_time > 0.0) {
        ...
        printf("Efficiency          : %.4f%%  (per SM, %d SMs)\n", efficiency_pct, prop.multiProcessorCount);
        printf("                      Note: GPU efficiency-per-SM is naturally much lower\n");
        printf("                      than CPU efficiency-per-thread -- a high Speedup with\n");
        printf("                      low %%-per-SM is expected and reflects the GPU's\n");
        printf("                      fundamentally different massively-parallel architecture.\n");
    } else {
        printf("Speedup             : N/A (pass serial_filter's total time as 4th arg to compute)\n");
